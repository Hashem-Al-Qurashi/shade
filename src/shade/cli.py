    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if latest_version != current_version:
                ...
                print(
                    f"[dim]You are on v{current_version}. Run [bold]git pull[/] to update.[/]\n"
                )
            elif not quiet:
...
@click.group(
    invoke_without_command=True, context_settings=dict(ignore_unknown_options=True)
)
...
def run_doctor(fix=False):
    ...
    print(
        f"Python version: [bold]{py_version.major}.{py_version.minor}.{py_version.micro}[/]"
    )
    if py_version < (3, 10):
    ...
    try:
        ...
        if free_gb < 10:
            print(
                "  [yellow]![/] Low disk space (under 10GB). Model downloads may fail."
            )
    except Exception:
        pass
    ...
    try:
        ...
        if mem.total < 16 * (1024**3):
            print(
                "  [yellow]![/] Low system RAM (under 16GB). Consider using quantization."
            )
    except ImportError:
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            print(
                "  [red]✗[/] No GPU or accelerator detected. CPU operations will be slow."
            )
    ...
    if not issues:
        ...
        print(
            f"[yellow bold]![/] System checks found {len(issues)} potential issue(s)."
        )
    ...
    if fix:
        ...
        print()
        print(
            "[green]✓[/] Auto-fix process completed. Please run [bold]shade doctor[/] again to verify."
        )
...
def run_install_cuda(url=False):
    ...
    print(
        "   [dim]pip install torch --upgrade --index-url https://download.pytorch.org/whl/cu121[/]"
    )
    print()
...
def commands():
    ...
    table.add_row(
        "[bold]shade[/]", "Start interactive model optimization (no arguments)"
    )
    ...
    table.add_row("", "")
    table.add_row(
        "[bold]shade download[/] <id>", "Download model from Hugging Face Hub"
    )
    table.add_row("", "")
    table.add_row(
        "[bold]shade cuda check[/]", "Check CUDA/GPU availability and version"
    )
    ...
    print()
    print(
        "[dim]Documentation is also available in [bold]COMMANDS.md[/] in the project root.[/]"
    )
    print(
        "[dim]For help, visit: [blue underline]https://github.com/AssemSabry/Shade[/][/]"
    )
...
def library():
    ...
    if click.confirm("Would you like to load a model from your library?"):
        ...
        if 1 <= choice <= len(saved_models):
            ...
            action = click.prompt(
                "Action", type=click.Choice(["chat", "serve", "cancel"]), default="chat"
            )
...
def ollama(model_path, name):
    ...
    try:
        subprocess.run(
            ["ollama", "create", name, "-f", str(modelfile_path)], check=True
        )
...
@click.option(
    "--perplexity", is_flag=True, help="Run Wikitext-2 perplexity evaluation."
)
@click.option(
    "--output",
...
def steer_train(model_id, output, layers, batch_size):
    ...
    from .config import Settings
    from .model import Model
    from .steering import save_steering_vector, train_steering_vector_from_prompts
...
def steer_evaluate(
    model_id, vector, multiplier, gsm8k, gsm8k_limit, perplexity, output
):
    """Benchmark a model with a steering vector active."""
    from .benchmark import format_benchmark_json, format_benchmark_table, run_benchmark
    from .config import Settings
    from .evaluator import Evaluator
    from .model import Model
    from .steering import load_steering_vector
    ...
    rprint(
        format_benchmark_json(result)
        if output == "json"
        else format_benchmark_table(result)
    )
...
@click.option(
    "--steering-multiplier", default=1.0, type=float, help="Steering strength."
)
@click.option(
    "--n-trials", default=30, type=int, help="Optuna trials for abliteration."
)
@click.option("--output", type=click.Choice(["table", "json"]), default="table")
def compare(
    model_id, gsm8k, gsm8k_limit, perplexity, steering_multiplier, n_trials, output
):
    """Run head-to-head comparison: base vs abliterated vs steered."""
    from .benchmark import run_benchmark
    from .compare import format_comparison_json, format_comparison_table
    from .config import Settings
    from .evaluator import Evaluator
    from .model import Model
    ...
    rprint(
        "[yellow]Abliteration step is not yet wired (abliterate_model not implemented).[/yellow]"
    )
    ...
    abl_result = run_benchmark(
        model=model_wrapper,
        evaluator=evaluator_abl,
        model_name="Abliterated",
        run_gsm8k=gsm8k,
        gsm8k_limit=gsm8k_limit,
        run_perplexity=perplexity,
    )
    ...
    sv = train_steering_vector_from_prompts(
        model=base_model,
        tokenizer=model_wrapper2.tokenizer,
        harmless_prompts=harmless,
        harmful_prompts=harmful,
    )
    ...
    with sv.apply(base_model, multiplier=steering_multiplier):
        ...
        steer_result = run_benchmark(
            model=model_wrapper2,
            evaluator=evaluator_steer,
    ...
    rprint(
        format_comparison_json(results)
        if output == "json"
        else format_comparison_table(results)
    )
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            if vram < 8:
                ...
                print(
                    "[dim]Suggesting 4-bit or 8-bit loading to prevent OOM errors.[/]"
                )
    except Exception:
        pass
...
def main():
    ...
    if len(sys.argv) <= 1 or (
        not sys.argv[1].startswith("-") and sys.argv[1] not in known_commands
    ):
        from .main import main as run_main
