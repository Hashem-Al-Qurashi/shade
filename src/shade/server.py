...
class ChatHistoryManager:
    ...
    def save_chat(model_name: str, messages: list):
        ...
        with open(log_file, "w", encoding="utf-8") as f:
            json.dump(
                {"model": model_name, "timestamp": timestamp, "messages": messages},
                f,
                ensure_ascii=False,
                indent=2,
            )
...
async def chat(request: ChatRequest):
    ...
    async def generate_chunks():
        ...
        streamer = TextIteratorStreamer(
            active_model.tokenizer, skip_prompt=True, skip_special_tokens=True
        )
...
async def compare(request: ChatRequest):
    ...
    try:
        ...
        original_response = active_model.stream_chat_response(
            chat_history
        )  # Corrected from active_model.str
        ...
        return {"original": original_response, "modified": modified_response}
...
def start_server(
    model_instance: Model, settings_instance: Settings, host="127.0.0.1", port=8000
):
    ...
    print()
    print("[bold green]🚀 Shade Web Server started![/]")
