...
def format_error_message(error: Exception) -> str:
    ...
    if isinstance(error, GatedRepoError) or (
        ...
        and ("gated repo" in error_msg.lower() or "401" in error_msg)
    ):
    ...
    if any(
        ...
        for keyword in ["no space left", "disk full", "storage full", "errno 28"]
    ):
    ...
    if any(
        keyword in error_msg.lower()
        for keyword in ["cuda", "gpu", "out of memory", "cudnn"]
    ):
...
def format_gated_repo_error(error_msg: str) -> str:
    ...
    lines.extend(
        [
            ...
            f"   [blue]https://huggingface.co/{model_name or 'the-model'}[/]"
            if model_name
            else "   https://huggingface.co/the-model",
            "",
